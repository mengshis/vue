...
export function createPatchFunction (backend) {
  ...
  function initComponent (vnode, insertedVnodeQueue) {
    ...
    if (isPatchable(vnode)) {
      ...
      setScope(vnode) // 为scoped CSS 设置scoped id
    } else {
  ...
  function invokeDestroyHook (vnode) {
    ...
    if (isDef(data)) {
      ...
      for (i = 0; i < cbs.destroy.length; ++i) cbs.destroy[i](vnode) // 调用destroy钩子
    }
    // 递归调用
  ...
  function removeVnodes (vnodes, startIdx, endIdx) {
    for (; startIdx <= endIdx; ++startIdx) {
      ...
      if (isDef(ch)) {
        if (isDef(ch.tag)) {
          // 存在tag时，移除节点并调用remove钩子函数
          removeAndInvokeRemoveHook(ch)
          invokeDestroyHook(ch)
        } else { // Text node
          //不存在tag则表明是一个text节点，直接移除即可
          removeNode(ch.elm)
        }
      }
    }
  ...
  function updateChildren (parentElm, oldCh, newCh, insertedVnodeQueue, removeOnly) {
    ...
    while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
      if (isUndef(oldStartVnode)) {
        ...
        /*新旧头尾比较的四种情况其实是指定key的时候，判定为同一个VNode，则直接patchVnode即可，分别比较oldCh以及newCh的两头节点2*2=4种情况*/
        ...
        canMove && nodeOps.insertBefore(parentElm, oldStartVnode.elm, nodeOps.nextSibling(oldEndVnode.elm)) // 插入到真实DOM中
        oldStartVnode = oldCh[++oldStartIdx]
        newEndVnode = newCh[--newEndIdx]
      } else if (sameVnode(oldEndVnode, newStartVnode)) { // Vnode moved left
        patchVnode(oldEndVnode, newStartVnode, insertedVnodeQueue, newCh, newStartIdx)
        canMove && nodeOps.insertBefore(parentElm, oldEndVnode.elm, oldStartVnode.elm) // 插入到真实DOM中
        oldEndVnode = oldCh[--oldEndIdx]
        newStartVnode = newCh[++newStartIdx]
  ...
  function patchVnode (
    ...
    if (isDef(vnode.elm) && isDef(ownerArray)) { // vnode.elm：虚拟节点对应的真实dom节点，ownerArray：自定义的节点
      // clone reused vnode
      vnode = ownerArray[index] = cloneVNode(vnode)
    }
    ...
    /*
    如果新旧VNode都是静态的，同时它们的key相同（代表同一节点，key被当作节点的标志），
  ...
  */
    ...
    if (isDef(data) && isDef(i = data.hook) && isDef(i = i.prepatch)) {
      //执行 vnode的 prepatch 钩子函数，最终是执行lifecycle中的updateChildComponent方法，更新子组件的slot、props、listener等
      i(oldVnode, vnode)
    }
    ...
    //如果有节点对象，并且节点可被更新（节点对应的最内层组件实例的vnode有tag属性）
    if (isDef(data) && isPatchable(vnode)) {
      /*调用update回调以及用户自定义的update钩子，所以虚拟dom渲染替换真实dom是发生在update生命周期里*/
      for (i = 0; i < cbs.update.length; ++i) cbs.update[i](oldVnode, vnode)
      if (isDef(i = data.hook) && isDef(i = i.update)) i(oldVnode, vnode)
    }
    /*如果这个VNode节点没有text文本时*/
    if (isUndef(vnode.text)) {
      if (isDef(oldCh) && isDef(ch)) {  // 新老节点均有children子节点，则对子节点进行diff操作
        if (oldCh !== ch) updateChildren(elm, oldCh, ch, insertedVnodeQueue, removeOnly)
      } else if (isDef(ch)) {
        // 如果老节点没有子节点而新节点存在子节点，表示旧节点不需要了。如果旧的节点是文本节点则先将节点的文本清除，然后通过 addVnodes 将 ch 批量插入到新节点 elm 下。
        if (process.env.NODE_ENV !== 'production') {
          checkDuplicateKeys(ch)
        }
        if (isDef(oldVnode.text)) nodeOps.setTextContent(elm, '')
        addVnodes(elm, null, ch, 0, ch.length - 1, insertedVnodeQueue)
      } else if (isDef(oldCh)) { // 如果新节点没有子节点而老节点有子节点的时候，则移除所有ele的子节点
        removeVnodes(oldCh, 0, oldCh.length - 1)
      } else if (isDef(oldVnode.text)) { // 当新老节点都无子节点的时候，清空文本
        nodeOps.setTextContent(elm, '')
      }
    } else if (oldVnode.text !== vnode.text) { // 当新老节点text不一样时，直接替换这段文本
      nodeOps.setTextContent(elm, vnode.text)
    }
    if (isDef(data)) {
      if (isDef(i = data.hook) && isDef(i = i.postpatch)) i(oldVnode, vnode) //执行 postpatch 钩子函数
    }
  }
  ...
  // 返回的patch函数，完成新老vnode比较和更新并将新的vnode渲染成真实dom
  return function patch (oldVnode, vnode, hydrating, removeOnly) {
    ...
    if (isUndef(oldVnode)) {
      ...
      if (!isRealElement && sameVnode(oldVnode, vnode)) {
        ...
        // 更新父的占位符节点
        if (isDef(vnode.parent)) { /*组件根节点被替换，遍历更新父节点element*/
